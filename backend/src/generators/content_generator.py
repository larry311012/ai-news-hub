...
class ContentGenerator:
    ...
    def __init__(self, ai_client, platform_config: Dict):
        ...
        Args:
            ai_client: AI client (from summarizer)
    ...
    def _generate_instagram_content(
        ...
        1. Caption (max 2200 chars, target 300-500 for engagement)
        2. Image prompt (for DALL-E generation)
        3. Hashtags (max 30, typically 5-10)
        ...
        try:
            # Generate caption
            caption = self._generate_instagram_caption(
                summary, config, user_id=user_id, post_id=post_id
            )

            # Generate image prompt for DALL-E
            image_prompt = self._generate_instagram_image_prompt(
                summary, user_id=user_id, post_id=post_id
            )
    ...
    def _generate_instagram_caption(
        ...
        """
        Generate Instagram caption with AI

        Instagram captions should be:
        - Engaging and conversational
        - 300-500 chars for optimal engagement (max 2200)
        - Include emojis
        - End with hashtags
        ...
        """
        caption_prompt = f"""Create an engaging Instagram caption for this AI/tech news:
...
- Instagram-friendly: visual-first mindset
...
Caption:"""
        ...
        caption = self._call_ai(
            ...
            context={
                "user_id": user_id,
                "post_id": post_id,
                "operation": "generate_instagram_caption",
            },
        )

        # Ensure it's not too long
        if len(caption) > 2200:
            logger.warning(f"Instagram caption too long ({len(caption)}), truncating")
            caption = caption[:2197] + "..."

        return caption.strip()

    def _generate_instagram_image_prompt(
        self, summary: Dict, user_id: Optional[int] = None, post_id: Optional[int] = None
    ) -> str:
        """
        Generate DALL-E image prompt from article content

        Image prompt should describe:
        - Visual concept related to the article
        - Style (modern, tech-focused, professional)
        - NO text, faces, or branded elements
        ...
        Returns:
            str: DALL-E prompt (max 400 chars)

        Raises:
            AIProviderError: If AI call fails
        """
        image_prompt_query = f"""Create a DALL-E image prompt for this AI/tech article:
...
- Specify style: photorealistic, 3D render, abstract art, or digital illustration

Example formats:
- "Modern abstract visualization of AI neural networks, vibrant blue and purple gradient, floating geometric shapes, high-tech aesthetic, 3D render"
- "Futuristic data center with glowing servers, cyberpunk style, neon blue accents, wide angle view, professional photography"
- "Abstract representation of machine learning, interconnected nodes with light particles, dark background, minimalist digital art"
...
Image Prompt:"""
        ...
        # Ensure it's not too long for DALL-E
        if len(image_prompt) > 400:
            logger.warning(f"Image prompt too long ({len(image_prompt)}), truncating")
            image_prompt = image_prompt[:397] + "..."
        ...
        try:
            ...
            if hasattr(self.ai_client, "provider"):
                context = {
                    ...
                    "post_id": post_id,
                    "platform": platform,
                    "operation": "generate_platform_post",
                }
    ...
    def _create_platform_prompt(self, platform: str, summary: Dict, config: Dict) -> str:
        ...
        # Validate tone
        valid_tones = ["casual", "professional", "formal", "conversational"]
        if tone not in valid_tones:
            logger.warning(f"Invalid tone '{tone}', using 'casual'")
            tone = "casual"

        hashtags = config.get("hashtags", [])
        # Validate hashtags
        if not isinstance(hashtags, list):
            logger.warning(f"Invalid hashtags type, using empty list")
            hashtags = []
...
"""
        ...
        platform_instructions = {
            ...
            "youtube": "Could be used as a community post or video description.",
        }
        ...
        # Extra warning for Threads
        if platform == "threads":
            base_prompt += f"\n\n⚠️ CRITICAL: Your response MUST be under {target_length} characters. Threads has a strict 500 character limit and we're targeting {target_length} to be safe. Count every character!"
    ...
    def _call_ai(
        ...
        try:
            ...
            ai_error = self.error_handler.handle_generic_error(
                e, getattr(self.ai_client, "provider", "unknown"), context
            )
            raise ai_error

    def format_with_emoji(self, text: str, platform: str) -> str:
        """Add appropriate emojis based on platform"""
        emoji_map = {
            "twitter": True,
            "threads": True,
            "instagram": True,
            "linkedin": False,  # More conservative
            "youtube": True,
        }

        if not emoji_map.get(platform, False):
            return text

        # Could enhance with AI-based emoji insertion
        return text
